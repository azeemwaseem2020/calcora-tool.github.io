...
function setStatus(message, ok = false) {
  ...
  status.style.color = ok ? "#087f70" : "#b04a3a";
}
...
async function loadComments() {
  ...
  if (error) {
    list.innerHTML = `<p class="comments-empty">Comments are temporarily unavailable. Please try again later.</p>`;
    return;
  }
  if (!data?.length) {
    list.innerHTML = `<p class="comments-empty">No comments yet. Be the first to leave a reply.</p>`;
    return;
  }
...
form.addEventListener("submit", async event => {
  ...
  if (website) return;
  if (name.length < 2 || name.length > 80) return setStatus("Please enter a valid name.");
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254) return setStatus("Please enter a valid email address.");
  if (comment.length < 2 || comment.length > 2000) return setStatus("Please enter a comment between 2 and 2000 characters.");
  ...
  if (error) {
    ...
    return setStatus("Could not post your comment. Please try again.");
  }
  const { error: contactError } = await supabase.from("comment_contacts").insert({ comment_id: data.id, email });
  if (contactError) console.warn("Private comment contact was not saved", contactError);
  form.reset();
  setStatus("Your comment has been posted publicly. Thank you!", true);
  button.disabled = false;
  button.textContent = "Post Comment";
  await loadComments();
});
